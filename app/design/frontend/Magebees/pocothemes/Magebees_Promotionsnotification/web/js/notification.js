    ...
    jQuery.widget('magebees.cwsnotification',{
        ...
        _create: function () {
			if (this.options.id=="bar") {
				 var bar_not = jQuery.cookie("dontShowBar");
				 var bar_once = jQuery.cookie("bar_once");
				if((!bar_not) || (!bar_once))
				{
				   if(!this.options.bar_after_time){
					   jQuery("#notification_slider").show();
					}
				}
			}
			
			 if (this.options.id=="popup") {
				 var popup_not = jQuery.cookie("dontShowPopup");
				 var popup_once = jQuery.cookie("popup_once");
				if((!popup_not) || (!popup_once))
				{
				   if(!this.options.popup_after_time){
					   jQuery("#popup").show();
					}
				}
			}
...
jQuery("#notification_slider").slideUp();}, this.options.bar_auto_close_time*1000);
        ...
        _displayBar: function () {
			...
			/* if (this.options.bar_all_or_one!="one") {
				var swiper = new Swiper('#swiper_notification_bar', {
				spaceBetween: 30,effect: 'fade',autoplay: {delay: 5000,},loop: loop,});     
			}*/
        ...
        _displayPopup: function () {
			...
			/*  if (this.options.popup_all_or_one!="one") {
				var swiper = new Swiper('#swiper_notification_popup', {
				spaceBetween: 30,effect: 'fade',autoplay: {delay: 5000,},loop: popup_loop,});     
			}*/
