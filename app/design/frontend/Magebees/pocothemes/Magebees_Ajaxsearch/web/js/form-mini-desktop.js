define([
    ...
    function isEmpty(value)
    {
     ...
     $.widget('magebees_ajaxquicksearch.ajaxQuickSearchDesktop', {
    ...
    var self=this;
            ...
            if (rtl==1) {
            $('#search_mini_form_desktop').addClass('rtl');
            }
            
            
            document.getElementById('search_loading_desktop').style['display']='none';
            $('#desktop_category').val(cat_sel).attr("selected", "selected");
            $('#desktop_category').change(function () {
                    $('#searchdesktop').val("");
                    
            });
            
            /* $("#desktop_category").each(function () {
            ...
            */
            $("#desktop_category").change(function () {
			   ...
			   if (selectedOption == null || $.trim(selectedOption) === ''){
				  ...
				  $("#cat_desktop_holder").text($.trim(selectedOption));
			   }
             ...
             this.element.click(function () {
                 ...
                 if (enable_recent_click==1) {
                     var recent_content=$(".recent_content").html();
                        ...
                        if (!searchtext) {
                        $('#search_autocomplete_desktop').html(recent_content);
                        }
                         if ($('#search_autocomplete_desktop').find('#ajax_recent_desktop').length) {
                         document.getElementById("search_autocomplete_desktop").style['display']='block';
                         }
                     }
                 }
            });
           ...
           this.element.on('focus', $.proxy(function () {
            ...
            var searchLabel = $('#search_mini_form_desktop .label');
        ...
        _onPropertyChange: function () {
              document.getElementById("search_autocomplete_desktop").style.display = "none";
            ...
            document.getElementById("search_autocomplete_desktop").innerHTML = "";
            ...
            if (value.length >= parseInt(this.options.minSearchLength, 10)) {
                var self=this;
                var cat_id=$('#desktop_category').val();
                    if (cat_id==undefined) {
                 ...
                 myTimeout = setTimeout(function () {
                    ...
                    document.getElementById('search_loading_desktop').style['display']='block';
                $.ajax({
                url: ajax_url,
                ...
                success: function (data) {
                    ...
                    document.getElementById("search_autocomplete_desktop").innerHTML = "";
					 ...
					 if (products) {
                        if (enable_product==1) {
              				 ...
              				 $('#search_autocomplete_desktop').append('<div id="mageb-search-auto-inner">');	
							if((enable_suggested==1)||(enable_popular==1)||(enable_recent==1)||(enable_category==1)||(enable_cms)==1){
							
                 			$('#mageb-search-auto-inner').append('<div id="search_autocomplete_desktop_first">');
							var search_result=$('#search_autocomplete_desktop_first');
							}
							else
							{								
								var search_result=$('#search_autocomplete_desktop'); 
							}
						}
						 else
						 {
                    ...
                    if (data.recent) {
                        $(search_result).append(data.recent);                      
                            self.highlightText(value,'#search_autocomplete_desktop .recent_item>a');
                        }
                    }
                    ...
                    if (data.popular) {
                        $(search_result).append(data.popular);                     
                            self.highlightText(value,'#search_autocomplete_desktop .popular_item>a');
                        }
                    }
                ...
                $(search_result).append('<ul id="ajax_suggest" class="searchTags"><li class="titleRow"><span class="order" style="display:none;">'+suggest_order+'</span><h6 id="suggested" class="mbSecTitle">'+$.mage.__(suggest_title)+'</li></ul>');
                            ...
                            var suggest_url=data.url+'?q='+suggest_text;
                                ...
                                $('#ajax_suggest').append('<li class="searchTag"><a href='+suggest_url+' id="title">'+suggest_data.title+'<span class="searchCount">'+suggest_data.num_results+'</span></a></h6></li></ul>');
                                                        self.highlightText(value,'#search_autocomplete_desktop .searchTag>a');
                                }
                            }
                            count++;
                            }
                    ...
                    if (enable_category==1) {
                        if (data.category) {
                            ...
                            self.highlightText(value,'#search_autocomplete_desktop .category_item>a');
                        }
                    }
                    if (enable_cms==1) {
                        if (data.cms) {
                            $(search_result).append(data.cms);                           
                            self.highlightText(value,'#search_autocomplete_desktop .cms_item>a');
                        }
                    }
                     ...
                     if (products) {
                        if (enable_product==1) {
                            if (cat_id==undefined) {
							 ...
							 $('#mageb-search-auto-inner').append('<div id="search_autocomplete_desktop_second">');                         
                             $('#search_autocomplete_desktop_second').append('<ul><li class="titleRow"><span class="order" style="display:none;">'+product_order+'</span><h6 class="mbSecTitle product_title">'+$.mage.__(product_title)+'<span><a href='+result_url+' id="title">'+$.mage.__("See All")+'</a></span></h6></li></ul>');
                            $('#search_autocomplete_desktop_second').append(data.products);
                            var searchtext=value;
							self.autoHeightProductSection();
                            self.highlightText(searchtext,'#search_autocomplete_desktop .ajxRightDetails > p');
                        }
                     }
                    if (!data.recent && !data.popular && !products && !data.title && !data.cms && !data.category) {
                        $('#search_autocomplete_desktop').append('<ul><li id="no_record">'+$.mage.__("No record Found")+'</li></ul>');
                     } 
					else if (!products && !data.title && !data.cms && !data.category) {
                    $('#search_autocomplete_desktop').append('<ul><li id="no_record">'+$.mage.__("No Search Results Found")+'</li></ul>');
                    }
                    self.sortUsingNestedText($('#search_autocomplete_desktop_first'),"ul","span.order");
					self.sortUsingNestedText($('#search_autocomplete_desktop_second'),"ul","span.order");
                    $('#search_autocomplete_desktop').trigger('contentUpdated');
                    self.element.removeAttr('aria-activedescendant');
                    document.getElementById("search_autocomplete_desktop").style.display = "block";
                    
                    var searchinput = $(".searchField #searchdesktop").width();
                    var searchautobox = $(".mbAutoSearch .search-autocomplete").width();
                    if (searchautobox > searchinput ) {
                        $(".mbAutoSearch").addClass("right");
                    }
                },				error: function (jqXHR, textStatus, errorThrown) {                  if (jqXHR.status == 500) {                      					  alert('Unexpected error, please check your search term.');                  } else {                      alert('Unexpected error, please check your search term.');                  }				  document.getElementById('search_loading_desktop').style['display']='none';				}
                });
                }, 1000);
            } else {
                this.autoComplete.hide();
    ...
    return $.magebees_ajaxquicksearch.ajaxQuickSearchDesktop;
});
