    ...
    $.widget('Magebees.swiperinit', {
        _create: function() {
            ...
            $(document).ajaxSuccess(function(event, xhr, settings) {
                ...
                if (
                    ajaxRequestURL.includes("/prodlist/index/ajaxload") ||
                    ajaxRequestURL.includes("/catthumb/index/ajaxload") ||
                    ajaxRequestURL.includes("/poco/index/blog") ||
                    ajaxRequestURL.includes("/imagegallery/index/ajaxload") ||
                    ajaxRequestURL.includes("/poco/index/todaydeal") ||
                    ajaxRequestURL.includes("?cat_ids=")) {
            ...
            [].forEach.call(nSlider, function(slider, index, arr) {
                ...
                if ((!jQuery.isEmptyObject(data)) && (!jQuery.isEmptyObject(carouseldata))) {
                    ...
                    if (slider.options.slider_id) {
						...
						//added for stop animation on page load in banner slider and animate on page slide
                        swiper.on('slideChangeTransitionEnd', function() {
                            
                            // Check if the current slider is inside a '.cwsRwdSlider' container
                            var cwsRwdSlider = $(slider).closest('.cwsRwdSlider');

                            // If the '.cwsRwdSlider' exists
                            if (cwsRwdSlider.length) {
                                // Get the slider ID from the Swiper instance (assuming it's stored in options.slider_id)
                                var sliderId = slider.options.slider_id;
                            
                                // Check if this specific slider has the slider_id attribute
                                if (sliderId) {
                                    //console.log('This slide belongs to the targeted cwsRwdSlider with slider_id:', sliderId); // Debugging log

                                    // Remove the 'cws-no-animation' class from the parent '.cwsRwdSlider' element
                                    cwsRwdSlider.removeClass('cws-no-animation');

                                    //console.log('cws-no-animation class removed from cwsRwdSlider with slider_id:', sliderId); // Debugging log
                                }
                            } 
                        });
