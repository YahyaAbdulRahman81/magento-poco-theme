...
class Configure extends \Magento\Wishlist\Controller\AbstractIndex
{
    ...
    public function execute()
    {
        ...
        try {
            ...
            $wishlistParams = new \Magento\Framework\DataObject();
            $wishlistParams->setCategoryId(false);
            $wishlistParams->setConfigureMode(true);
            $wishlistBuyRequest = $item->getBuyRequest();
            if (!$wishlistBuyRequest->getQty() && $item->getQty()) {
                $wishlistBuyRequest->setQty($item->getQty());
            }
            if ($wishlistBuyRequest->getQty() && !$item->getQty()) {
                $item->setQty($wishlistBuyRequest->getQty());
                $this->_objectManager->get('Magento\Wishlist\Helper\Data')->calculate();
            }
            $wishlistParams->setBuyRequest($wishlistBuyRequest);
