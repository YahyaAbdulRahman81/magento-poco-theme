...
class Categoryurl implements ObserverInterface
{
	...
	public function execute(\Magento\Framework\Event\Observer $observer)
    {
		...
		$blog_category = $this->blogcategory->getCollection()
									->addFieldToFilter('category_id', array('neq' => $category_id))
									->addFieldToFilter('identifier', array('eq' => $identifier))
									->getFirstItem();
		...
		$blog_post = $this->blogpost->getCollection()
									->addFieldToFilter('identifier', array('eq' => $identifier))
									->getFirstItem();
		$blog_post_id = $blog_post->getCategoryId();
		if($blog_post_id):
		...
		$blogurlrewrite->addFieldToFilter('type', array('neq' => 'category'));
