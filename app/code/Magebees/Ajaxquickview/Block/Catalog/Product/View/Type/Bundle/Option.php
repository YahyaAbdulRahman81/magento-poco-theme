...
namespace Magebees\Ajaxquickview\Block\Catalog\Product\View\Type\Bundle;
...
class Option extends \Magento\Bundle\Block\Catalog\Product\Price
{
    ...
    public function __construct(
        ...
        \Magebees\Ajaxquickview\Helper\Data $helper1,
    ...
    public function getSelectionQtyTitlePrice($selection, $includeContainer = true)
    {
        ...
        $priceTitle .=$this->ImageIcon($selection);    
    ...
    public function ImageIcon($value)
    {
         $ImageIcon='';
         if($this->helper1->getConfigValues()->getValue('customstockstatus/general/enable', \Magento\Store\Model\ScopeInterface::SCOPE_STORE) && $this->helper1->getConfigValues()->getValue('customstockstatus/display_settings/other_view', \Magento\Store\Model\ScopeInterface::SCOPE_STORE)){

         $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
         $helper=$objectManager->create('Magebees\Customstockstatus\Helper\Data');   
         ...
         $status = $helper->getStatus($value->getSku());
         if ($status=='')
                   {
                      $status=$helper->getRuleStatus($value,'product');    
                   }
         
          if ($enable && $showproductpage) {
            ...
            for ($i=1; $i < count($attribute); $i++) {       
                if ($attribute[$i]['value']==$status) {
                    foreach ($icon as $ic) {
                        ...
                        $ImageIcon.='<div id="'.$value->getSelectionId().'"><img  src="'.$path.$ic["icon_name"].'" max-width="100%" max-height="100%" /><br>';
                        $objectManager = \Magento\Framework\App\ObjectManager::getInstance();
                          $StockState = $objectManager->get('\Magento\CatalogInventory\Api\StockStateInterface');
                          $qty=$StockState->getStockQty($value->getId(), $value->getStore()->getWebsiteId());  
                          
                          if (strpos($attribute[$i]['label'], '{qty}') !== false) {
                            if (!$icon_text) {
                              
                            $ImageIcon.=str_replace("{qty}",$qty,$attribute[$i]['label']).'</div>';
                        ...
                        {     
                            if (!$icon_text) {
                              $ImageIcon.= $attribute[$i]['label'].'</div>';
                             }
                             else
                             {
                                $ImageIcon.='</div>';
